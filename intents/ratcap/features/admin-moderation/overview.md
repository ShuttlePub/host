...
- `apps/emumet-web/bff/schema.graphql` に既存の `Moderation` 型を使用し、 `suspendAccount(id: ID!, reason: String!, expiresAt: DateTime): Boolean!` 、 `unsuspendAccount(id: ID!): Boolean!` 、 `banAccount(id: ID!, reason: String!): Boolean!` mutation が追加される。
- BFF コンテキストに admin role 判定を追加し、admin 以外のリクエストは `FORBIDDEN` または `UNAUTHENTICATED` エラーを返す。
- `apps/emumet-web/bff/emumet/client.ts` の `EmumetClient` 契約に `suspendAccount` / `unsuspendAccount` / `banAccount` メソッドが追加され、`real.ts` / `mock.ts` の両方が実装される。
- `apps/emumet-web/bff/resolvers.ts` に admin-only のリゾルバが実装される。
...
- `bun test`（apps/emumet-web 配下） の BFF テストに admin 判定付きリゾルバテストが追加され全て通る。

...
1. **GraphQL スキーマ拡張**
   - `apps/emumet-web/bff/schema.graphql` の `Mutation` 型に `deleteAccount(id: ID!): Boolean!` を追加する。
   - 入力 `id` は対象アカウントの Emumet account ID 文字列とする。
   - 戻り値は削除が成功した場合 `true`、失敗した場合は GraphQL エラーを返す。

2. **EmumetClient 契約拡張**
   - `apps/emumet-web/bff/emumet/client.ts` の `EmumetClient` インターフェースに `deleteAccount(id: string): Promise<void>` を追加する。
   - 実装は `apps/emumet-web/bff/emumet/real.ts` と `apps/emumet-web/bff/emumet/mock.ts` の両方に行う。
...
3. **リゾルバ実装**
   - `apps/emumet-web/bff/resolvers.ts` に `Mutation.deleteAccount` を実装する。
   - 未認証なら `requireEmumet` で `UNAUTHENTICATED` エラーを返す。
   - 成功時は `true`、失敗時は既存の `withEmumetErrors` / `toGraphQLError` 写像を経由してエラーを返す。

4. **PureScript 型再生成**
   - `(cd apps/emumet-web && bun scripts/sync-graphql.ts)` を実行し、`apps/emumet-web/src/Generated/` 配下の型を再生成する。
   - 生成後に `spago build` が成功する。

5. **UI 実装**
   - `apps/emumet-web/src/App/View/AccountDetail.purs` の下部に「危険領域 (Danger Zone)」セクションを追加する。
...
6. **状態管理**
   - `apps/emumet-web/src/App/Message.purs` に `DeleteAccount String` / `DeleteAccountConfirmed String` / `AccountDeleted String` / `AccountDeleteFailed String String` などのメッセージを追加する（命名は実装時に調整）。
   - `apps/emumet-web/src/App/Model.purs` に確認ダイアログの開閉状態と入力中の確認文字列を保持するフィールドを追加する。
...
- **テスト**: BFF リゾルバの成否ケース（認証あり成功、未認証、存在しない ID、所有権違反）を `apps/emumet-web/bff/*.test.ts` でカバーする。

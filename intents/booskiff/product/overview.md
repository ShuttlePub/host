...
## 課金・容量 (grill 第2ラウンド 2026-09-06 確定、Q23-Q31)

- **プラン**: free 5 GB / premium 100 GB・980円/月 + 追加ストレージオプション
  (+50GB・500円/月 単位)。単ファイル 100 MB・100 req/min は全 tier 共通
- **over-quota**: 容量超過・降格時は read-only 縮退。アップロード・新規フォルダ・
  新規公開のみブロックし、既存ファイルの DL・削除・既存公開 URL 配信は継続。
  ユーザーデータの強制削除はしない
- **ゴミ箱**: 削除で 7 日間保持 (容量計上あり)。ゴミ箱内は公開配信停止、
  復元で公開設定も戻る。ユーザーは即時完全削除も可能
- **購入モデル**: 前払い期間制 (1/3/6/12ヶ月、期限前リマインダー、支払いで延長)。
  決済は BTCPay (BTC) 主軸 + PayPal + PayPay (国内単発)。期限切れは即降格
- **課金画面**: 容量・プラン・期限・購入導線は booskiff-web の課金ページ。
  運営者向けコントロールパネル (課金 ON/OFF、プラン手動割当) も booskiff-web 内
- **セルフホスト**: 既定 mirror モード (全員 free 相当)。課金プロバイダ設定も
  管理画面から行え、公式インスタンスと同じ運用が可能

設計根拠・損益モデルは `../decisions/2026-08-29-initial-shaping.md` (D22-D28)、
詳細は `../interviews/booskiff.json` (Q23-Q31)。

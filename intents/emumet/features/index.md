# emumet — features

> **Ask intent-cli first:** `intent-cli guide intent-work setup --kind tree-layout --domain emumet --format markdown`

## Feature list

- [account-management](account-management/overview.md)
- [ap-federation](ap-federation/overview.md)
- [post-relay](post-relay/overview.md)
- [shuttlepub-link](shuttlepub-link/overview.md)
- [block-mute](block-mute/overview.md)
- [media-upload](media-upload/overview.md)
- [moderation](moderation/overview.md)
- [org-accounts](org-accounts/overview.md) —
  [packets](org-accounts/packets.md) /
  [open questions](org-accounts/open-questions.md)

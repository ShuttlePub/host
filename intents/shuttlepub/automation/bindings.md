---
# Automation bindings for the `shuttlepub` intent domain (tree-v1).
#
...
child_repo: ShuttlePub/ShuttlePub
...
# single-domain host). Narrow it (for example `^shuttlepub-`) once you
# share a `.intent-cli/issues` root across multiple domains.
execution_unit_regex: .*
---

# shuttlepub automation bindings

This file maps the `shuttlepub` intent domain to its implementation
...
- `intent-cli intent host-check --domain shuttlepub --format json`
- `intent-cli intent next-slice --dry-run --domain shuttlepub --format json`
- `intent-cli automation summary --domain shuttlepub --format json`
